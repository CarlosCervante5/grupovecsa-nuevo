...
class VehicleService
{
    ...
    public function __construct(UserService $userService, DealershipAccessService $dealershipAccess)
    {
     ...
     */
    public function createVehicle($data, User $user)
    {
        ...
        if (! $vehicle->exists) {
            ...
            $this->dealershipAccess->assertDealershipAllowed($user, $dealership->id);
            $brand = VehicleBrand::firstOrCreate(['name' => strtolower($data['brand'])]);
            $line = BrandLine::firstOrCreate(['name' => strtolower($data['line']), 'brand_id' => $brand->id]);
            $model = LineModel::firstOrCreate(['name' => strtolower($data['model']), 'year' => $data['year'], 'line_id' => $line->id]);
        ...
        $this->userService->vehicleUpdate('Vehicle Controller: Create vehicle('.$exists.')', null, json_encode($data), $user->id, $vehicle->id);
     ...
     * @return Vehicle El vehículo creado o actualizado.
     */
    public function createOrUpdateVehicle($data, User $user)
    {
        // Crear o obtener las entidades relacionadas
        $dealership = Dealership::firstOrCreate(['name' => strtolower($data['dealership_name']), 'location' => strtolower($data['location'])]);
        $this->dealershipAccess->assertDealershipAllowed($user, $dealership->id);
        ...
        $this->userService->vehicleUpdate('Vehicle Controller: '.$exists.' vehicle', json_encode([$vehicle, ($vehicleSpecification ?? null)]), json_encode($data), $user->id, $vehicle->id);
     ...
     */
    public function updateStatus($data, User $user)
    {
        ...
        $this->userService->vehicleUpdate('Vehicle Controller: Update vehicle', json_encode([$vehicle, null]), json_encode($data), $user->id, $vehicle->id);
     ...
     */
    public function deleteVehicle($uuid, User $user)
    {
        ...
        $this->userService->vehicleUpdate('Vehicle Controller: Delete vehicle', null, json_encode($uuid), $user->id, $vehicle->id);
     ...
     * @return bool Verdadero si el vehículo fue eliminado, falso si no existe.
     */
    public function deleteVehicleBatch($uuids, User $user)
    {
        $scopeIds = $this->dealershipAccess->inventoryDealershipIds($user);
        $q = Vehicle::whereIn('uuid', $uuids);
        if ($scopeIds !== null) {
            $q->whereIn('dealership_id', $scopeIds);
        }
        $vehiclesExist = $q->exists();

        if ($vehiclesExist) {

            $q->delete();

            $this->userService->vehicleUpdate('Vehicle Controller: Delete vehicle batch', null, json_encode($uuids), $user->id);
     ...
     * @return bool Verdadero si el vehículo fue eliminado, falso si no existe.
     */
    public function inverseDeleteVehicleBatch($uuids, User $user)
    {
        $scopeIds = $this->dealershipAccess->inventoryDealershipIds($user);
        $q = Vehicle::whereNotIn('uuid', $uuids);
        if ($scopeIds !== null) {
            $q->whereIn('dealership_id', $scopeIds);
        }
        $vehiclesExist = $q->exists();
        ...
        if ($vehiclesExist) {
            ...
            $this->userService->vehicleUpdate('Vehicle Controller: Inverse delete vehicle batch', null, json_encode($uuids), $user->id);
     ...
     * @return bool Verdadero si el vehículo fue eliminado, falso si no existe.
     */
    public function statusVehicleBatch($uuids, $page_status, User $user)
    {
        $scopeIds = $this->dealershipAccess->inventoryDealershipIds($user);
        $q = Vehicle::whereIn('uuid', $uuids);
        if ($scopeIds !== null) {
        ...
        if ($vehiclesExist) {
            ...
            $q->update(['page_status' => $page_status]);

            $this->userService->vehicleUpdate('Vehicle Controller: Status vehicle batch ('.$page_status.')', null, json_encode($uuids), $user->id);
     ...
     */
    public function restoreVehicle($uuid, User $user)
    {
        ...
        $this->userService->vehicleUpdate('Vehicle Controller: Restore vehicle', null, json_encode($uuid), $user->id, $vehicle->id);
     ...
     */
    public function searchVehicles($data, ?User $user = null)
    {
    ...
    public function searchVehiclesXML()
    {
        $data = [
            'channel' => [
                'title' => 'Inventario de Vecsa Hidalgo',
                'description' => 'Inventario de Vecsa Hidalgo para Facebook',
            ],
        ];
        ...
        foreach ($vehicles as $key => $vehicle) {
            ...
            $product = [
                ...
                'link' => "https://grupovecsa.com/inventory/compra-tu-auto/detail/$vehicle->uuid",
     ...
     */
    public function minMaxPrices($data, ?User $user = null)
    {
        $query = Vehicle::query()->where($this->statusCondition($data['status']));
        $scopeIds = $this->dealershipAccess->inventoryDealershipIds($user);
        if ($scopeIds !== null) {
            $query->whereIn('dealership_id', $scopeIds);
        }
     ...
     * @return \Illuminate\Contracts\Pagination\LengthAwarePaginator Vehículos encontrados.
     */
    public function randomSearchVehicles($data, ?User $user = null)
    {
        // Crear la consulta base
        $query = Vehicle::query();
        ...
        $fallback = Vehicle::with($data['relationship_names'])
            ->where($this->statusCondition($data['status']))
            ->where($this->pricesCondition($data['prices']))
            ->whereHas('images');
        if ($scopeIds !== null) {
            $fallback->whereIn('dealership_id', $scopeIds);
        }

        return $fallback->inRandomOrder()->limit(10)->get();
