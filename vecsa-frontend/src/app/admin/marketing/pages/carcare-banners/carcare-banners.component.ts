...
import { CarCareBanner, CarCareBannersResponse } from '@interfaces/admin.interfaces';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-carcare-banners',
  templateUrl: './carcare-banners.component.html',
  styleUrls: ['./carcare-banners.component.css'],
  standalone: false
})
export class CarcareBannersComponent {

  public banners: CarCareBanner[] = [];
  public loading = true;
  public showForm = false;
  public editingBanner: CarCareBanner | null = null;
  public saving = false;
  ...
  constructor(
    private _bannerService: CarCareBannerService,
  ...
  loadBanners(): void {
    ...
    this._bannerService.search().subscribe({
      next: (response: CarCareBannersResponse) => {
  ...
  openEditForm(banner: CarCareBanner): void {
    this.editingBanner = banner;
    this.title = banner.title || '';
    this.subtitle = banner.subtitle || '';
    this.disclaimer = banner.disclaimer || '';
  ...
  toggleActive(banner: CarCareBanner): void {
  ...
  deleteBanner(banner: CarCareBanner): void {
    Swal.fire({
  ...
  drop(event: CdkDragDrop<CarCareBanner[]>): void {
