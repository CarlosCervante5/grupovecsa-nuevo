...
@Component({
    selector: 'app-new-riderkm',
    templateUrl: './new-riderkm.component.html',
    styleUrls: ['./new-riderkm.component.css'],
    standalone: false
})

export class NewRiderkmComponent{
    ...
    private async getRewards() {
        ...
        this._rewards.getRewardsByCategory('ride')
            .subscribe({
            next: ( rewardsResponse: RewardsResponse) => {
                this.rewards = rewardsResponse.data;

                this.form.patchValue({
                    reward_uuid : this.rewards[0].uuid
                });
            }
        });
    }
