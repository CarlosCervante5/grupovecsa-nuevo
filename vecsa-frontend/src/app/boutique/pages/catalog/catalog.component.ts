...
import { BoutiqueBannerService } from '../../../shared/services/boutique-banner.service';
import {
  BoutiqueProduct,
  BoutiqueCategory,
...
export class CatalogComponent implements OnInit, OnDestroy {
  ...
  featuredProducts: BoutiqueProduct[] = [];
  ...
  heroSlides: HeroSlide[] = [
    { title: 'Boutique BMW', subtitle: 'Mercancía oficial, accesorios y estilo de vida BMW', cta: 'Explorar colección', bg: 'slide-1' },
    { title: 'Accesorios Premium', subtitle: 'Equipa tu BMW con los mejores accesorios originales', cta: 'Ver accesorios', bg: 'slide-2' },
    { title: 'Estilo de Vida', subtitle: 'Ropa, calzado y artículos de colección BMW', cta: 'Descubrir más', bg: 'slide-3' },
  ];
  ...
  constructor(
    ...
    private bannerService: BoutiqueBannerService,
  ...
  loadBanners(): void {
    this.bannerSub = this.bannerService.publicList().subscribe({
      next: (res) => {
        const banners = res?.data?.banners;
        if (Array.isArray(banners) && banners.length > 0) {
          this.heroSlides = banners.map((b: any) => ({
            title: b.title || '',
            subtitle: b.subtitle || '',
            cta: b.cta_text || 'Explorar',
            bg: b.bg_class || 'slide-1',
      ...
      error: () => { /* keep default slides on error */ },
    });
  }
  ...
  loadFeatured(): void {
    ...
    this.featuredSub = this.catalogService.search({ page: 1, per_page: 10 }).subscribe({
      next: (res) => {
        const d = res?.data as any;
        const paginated = d?.products ?? d;
        const items = paginated?.data;
        this.featuredProducts = Array.isArray(items) ? [...items] : [];
