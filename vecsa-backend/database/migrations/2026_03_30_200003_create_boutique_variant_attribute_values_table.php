...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable(env('DB_TABLE_PREFIX', '') . 'boutique_variant_attribute_values')) { return; }
        Schema::create(env('DB_TABLE_PREFIX', '') . 'boutique_variant_attribute_values', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('variant_id');
            $table->unsignedBigInteger('attribute_value_id');

            $table->foreign('variant_id', 'bvav_variant_id_fk')
                  ->references('id')
                  ->on(env('DB_TABLE_PREFIX', '') . 'boutique_product_variants')
                  ->onDelete('cascade');

            $table->foreign('attribute_value_id', 'bvav_attr_value_id_fk')
                  ->references('id')
